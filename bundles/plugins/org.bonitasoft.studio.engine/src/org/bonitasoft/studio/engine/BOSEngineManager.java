...
public class BOSEngineManager {
	...
	public void logoutPlatform(PlatformSession session) throws PlatformLogoutException, SessionNotFoundException,
	...
	public APISession createSession(final AbstractProcess process, final String configurationId,
		...
		String password = retrieveUserPasswordFromActiveOrga(username).orElseThrow(() -> new Exception(
				String.format("Unable to retrieve the password of %s in the active organization.", username)));
		try {
			session = BOSEngineManager.getInstance().loginTenant(username, password, monitor);
		} catch (final Exception e1) {
			throw new Exception(Messages.bind(Messages.loginFailed,
					new String[] { username, process.getName(), process.getVersion() }), e1);
		}
		if (session == null) {
			throw new Exception(Messages.bind(Messages.loginFailed,
					new String[] { username, process.getName(), process.getVersion() }));
		}
		return session;
	}
	...
	public byte[] getTenantConfigResourceContent(String resourceName, IProgressMonitor monitor)
		...
		try {
			...
			var clientTenantConfigurations = platformAPI.getClientTenantConfigurations();
			if (!clientTenantConfigurations.containsKey(resourceName)) {
				throw new FileNotFoundException(String.format("Resource %s does not exist in database.", resourceName));
			}
			return clientTenantConfigurations.get(resourceName);
		} finally {
