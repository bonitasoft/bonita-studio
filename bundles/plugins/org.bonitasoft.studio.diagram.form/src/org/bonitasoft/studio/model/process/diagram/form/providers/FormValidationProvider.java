	...
	public static void runWithConstraints(TransactionalEditingDomain editingDomain, Runnable operation) {
		...
		if (editingDomain != null) {
			try {
				...
				FormDiagramEditorPlugin.getInstance().logError("Validation failed", e); //$NON-NLS-1$
			}
		} else {
			task.run();
		}
	...
	*/
	static boolean isInDefaultEditorContext(Object object) {
		...
		if (object instanceof View) {
			return constraintsActive && FormEditPart.MODEL_ID.equals(ProcessVisualIDRegistry.getModelID((View) object));
		}
		return true;
	}
