 ...
 */
package org.bonitasoft.studio.model.process.diagram.form.expressions;
...
public abstract class ProcessAbstractExpression {
	...
	*/
	protected void setStatus(int severity, String message, Throwable throwable) {
		String pluginID = FormDiagramEditorPlugin.ID;
		this.status = new Status(severity, pluginID, -1, (message != null) ? message : "", throwable); //$NON-NLS-1$
		if (!this.status.isOK()) {
			FormDiagramEditorPlugin.getInstance().logError("Expression problem:" + message + "body:" + body(), //$NON-NLS-1$//$NON-NLS-2$
					throwable);
		}
	}
	...
	public Object evaluate(Object context, Map env) {
		if (context().isInstance(context)) {
			try {
				...
				FormDiagramEditorPlugin.getInstance().logError("Expression evaluation failure: " + body(), e); //$NON-NLS-1$
			}
		}
		return null;
	}
