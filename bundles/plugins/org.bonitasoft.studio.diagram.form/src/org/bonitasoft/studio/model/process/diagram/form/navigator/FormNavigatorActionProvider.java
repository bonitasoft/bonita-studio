 ...
 * @generated
 */
public class FormNavigatorActionProvider extends CommonActionProvider {
	...
	private static class OpenDiagramAction extends Action {
		...
		*/
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof FormNavigatorItem) {
					selectedElement = ((FormNavigatorItem) selectedElement).getView();
				} else if (selectedElement instanceof IAdaptable) {
					selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
				}
				if (selectedElement instanceof Diagram) {
					Diagram diagram = (Diagram) selectedElement;
					if (FormEditPart.MODEL_ID.equals(ProcessVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		*/
		public void run() {
			...
			try {
				page.openEditor(editorInput, FormDiagramEditor.ID);
			} catch (PartInitException e) {
				FormDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e); //$NON-NLS-1$
			}
		}
