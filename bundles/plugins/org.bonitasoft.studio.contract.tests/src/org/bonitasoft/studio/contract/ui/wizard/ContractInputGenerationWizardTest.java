...
public class ContractInputGenerationWizardTest {
    ...
    @BeforeClass
    public static void loadBonitaGroovyEditor() throws Exception {
        try {
            new BonitaGroovyEditor();
        } catch (Throwable e) {
            // Ignore error: workbench not created
        }
    }
    ...
    public void should_add_a_contract_input_with_selected_mappings_on_finish() throws Exception {
        ...
        final ContractInputGenerationWizard wizard = new ContractInputGenerationWizard(process, editingDomain(),
                repositoryAccessor, operationBuilder,
                expressionBuilder,
                contractConstraintBuilder, preferenceStore, sharedImages, dialogFactory,
    ...
    public void should_canFinish_return_true_when_data_is_selected() {
        final Pool process = aPool().havingContract(aContract()).build();
        final BusinessObjectData data = aBusinessData().withName("employee").withClassname("com.company.Employee")
                .build();
        process.getData().add(data);
    ...
    public void should_take_into_account_edit_mode_on_tasks() throws Exception {
        ...
        verify(generator, times(0)).build(any(BusinessObjectData.class), eq(EditMode.EDIT),
