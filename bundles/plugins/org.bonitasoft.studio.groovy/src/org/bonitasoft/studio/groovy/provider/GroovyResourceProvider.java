...
public class GroovyResourceProvider implements IBOSArchiveFileStoreProvider {
     ...
     */
    @Override
    public Set<IRepositoryFileStore<?>> getFileStoreForConfiguration(AbstractProcess process,
            Configuration configuration) {
        final Set<IRepositoryFileStore<?>> files = new HashSet<>();

        final GroovyRepositoryStore groovySotre = (GroovyRepositoryStore) RepositoryManager.getInstance()
    ...
    protected void addScriptFile(final Set<IRepositoryFileStore<?>> files, final GroovyRepositoryStore groovySotre,
            FragmentContainer groovyContainer) {
