...
public class GroovyDocumentUtil {
                ...
                if(type != null){
                    for(IMethod m :type.getMethods()){
                        if(m.getFlags() == (Flags.AccStatic | Flags.AccPublic) ){
                            FunctionsRepositoryFactory.getUserFunctionCatgory().addFunction(new GroovyFunction(m,FunctionsRepositoryFactory.getUserFunctionCatgory()));
                        }
                    }
                }
            }
    ...
    public synchronized static List<String> getDefaultMethodsName() {
        if(defaultMethodsNames == null){
            defaultMethodsNames = new ArrayList<String>();
    ...
    //	public static void deleteGroovyType(DataType dataType) {
    //		GroovyScriptRepository.getInstance().deleteGroovyType(dataType);
    //	}
    //
    //	public static void createGroovyType(DataType dataType, List<String> literals) {
    //		GroovyScriptRepository.getInstance().createGroovyTypes(dataType,literals);
    //		ExtensionProjectUtil.refresh(new NullProgressMonitor());
    //	}
