...
public class InstallOrganizationHandler extends AbstractHandler {
    ...
    public Object execute(final ExecutionEvent event) throws ExecutionException {
        if (event != null) {
            ...
            if (file == null) {
                BonitaStudioLog.warning("Organization : " + id + " not found !", IdentityPlugin.PLUGIN_ID);
                final List<? extends IRepositoryFileStore> organizationFiles = organizationStore.getChildren();
                if (organizationFiles.isEmpty()) {
                    BonitaStudioLog.warning("No organization found in repository", IdentityPlugin.PLUGIN_ID);
                    return null;
                } else {
