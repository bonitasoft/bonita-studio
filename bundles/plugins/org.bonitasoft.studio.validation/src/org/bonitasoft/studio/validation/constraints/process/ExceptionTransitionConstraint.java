...
public class ExceptionTransitionConstraint extends AbstractLiveValidationMarkerConstraint {
	...
	protected IStatus performLiveValidation(IValidationContext ctx) {
		...
		if(feature.equals(ProcessPackage.Literals.SOURCE_ELEMENT__OUTGOING)){
			BoundaryEvent event = (BoundaryEvent) ctx.getTarget();
			int size = event.getOutgoing().size();
			if(size == 0){
	...
	protected IStatus performBatchValidation(IValidationContext ctx) {
		BoundaryEvent target = (BoundaryEvent) ctx.getTarget();
		int size = target.getOutgoing().size();
		if(size == 0){
			return ctx.createFailureStatus(Messages.validation_exceptionTransitionError_noTransition);
		} else if(size == 1){
			return ctx.createSuccessStatus();
