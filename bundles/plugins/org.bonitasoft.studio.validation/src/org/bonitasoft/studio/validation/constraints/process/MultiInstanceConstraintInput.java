...
public class MultiInstanceConstraintInput extends AbstractLiveValidationMarkerConstraint {
    ...
    private IStatus validateMultiInstantiation(final IValidationContext ctx, final MultiInstantiable multiInstantiable) {
        if (!multiInstantiable.isUseCardinality()
                && multiInstantiable.getIteratorExpression() != null
                && multiInstantiable.getIteratorExpression().getContent() != null
                && !multiInstantiable.getIteratorExpression().getContent().isEmpty()
                && multiInstantiable.getCollectionDataToMultiInstantiate() == null) {
