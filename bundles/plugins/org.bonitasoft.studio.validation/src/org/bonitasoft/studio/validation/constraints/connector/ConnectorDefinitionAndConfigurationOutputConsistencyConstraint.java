...
public class ConnectorDefinitionAndConfigurationOutputConsistencyConstraint extends AbstractLiveValidationMarkerConstraint {
	...
	protected IStatus performBatchValidation(IValidationContext context) {
		...
		if (def!=null){
			...
			if(!outputStatus.isOK()){
				if(isConnector){
					return context.createFailureStatus( Messages.bind(Messages.Validation_InconsistentConnectorDefAndConfigurationOutput,new Object[]{connector.getName(),connector.getDefinitionId()+"--"+connector.getDefinitionVersion(),outputStatus.getMessage()}));
				}else{
					return context.createFailureStatus( Messages.bind(Messages.Validation_InconsistentActorDefAndConfigurationOutput,new Object[]{connector.getName(),connector.getDefinitionId()+"--"+connector.getDefinitionVersion(),outputStatus.getMessage()}));
				}
			}
		}
		return context.createSuccessStatus();
	...
	protected IStatus checkOutputConsistency(Connector connector,ConnectorDefinition def, IValidationContext context) {
		...
		Set<String> connectorOutputs = new HashSet<String>(); 
