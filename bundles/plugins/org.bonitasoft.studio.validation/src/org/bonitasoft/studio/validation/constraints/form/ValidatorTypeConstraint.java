        ...
        if(feature.equals(FormPackage.Literals.VALIDATOR__VALIDATOR_CLASS)){
            ...
            if(ctx.getEventType().equals(EMFEventType.ADD)){
                ...
                if(validator instanceof Validator){
                    ...
                    if (validatorClass == null || validatorClass.trim().isEmpty()) {
                        return ctx.createFailureStatus(new Object[] { Messages.Validation_Validator_EmptyValidatorType+" "+((Validator) validator).getName()});
                    }
                }
            }
        }
    ...
    protected IStatus performBatchValidation(IValidationContext ctx) {
        ...
        if(target instanceof Validator){
            ...
            if (validatorClass == null || validatorClass.isEmpty()) {
                ...
                ValidatorDescriptorRepositoryStore validatorStore = (ValidatorDescriptorRepositoryStore) RepositoryManager.getInstance().getRepositoryStore(ValidatorDescriptorRepositoryStore.class) ;
                if(validatorStore.getValidatorDescriptor(validatorClass) == null){
                    return ctx.createFailureStatus(new Object[] { Messages.bind(Messages.Validation_Validator_MissingValidatorType,validatorClass) + " " + v.getName() });
                }
            }
        }
        return ctx.createSuccessStatus();
