...
public class ConnectorDefinitionAndConfigurationInputConsistencyConstraintTest {
	...
	public void shouldCheckInputConsistency_ReturnOkStatus() throws Exception {
		ConnectorConfiguration config = connectorConfigFactory.createConnectorConfiguration();
	...
	private ConnectorParameter createConnectorParameter(String inputName) {
		ConnectorParameter connectorParameter = connectorConfigFactory.createConnectorParameter();
		connectorParameter.setKey(inputName);
		connectorParameter.setExpression(null);
		return connectorParameter;
	}

	private Input createInput(String name, String returnType) {
		Input input = connectorDefFactory.createInput();
		input.setName(name);
		input.setType(returnType);
		return input;
	}
	
	private Input createInput(String name, String returnType,boolean isMandatory) {
		Input input = createInput(name, returnType);
		input.setMandatory(isMandatory);
		return input;
	}
	
	
	private Output createOutput(String name, String returnType) {
		Output connectorOutput = connectorDefFactory.createOutput();
		connectorOutput.setName(name);
		connectorOutput.setType(returnType);
		return connectorOutput;
	}

	private Operation createOutputOperation(String leftOperand, String rightOperand) {
	...
	private Expression createExpression(String leftOperand) {
		Expression expression = expressionFactory.createExpression();
		expression.setContent(leftOperand);
		return expression;
	}
	
	private Expression createConnectotOutputExpression(String content) {
		Expression expression = expressionFactory.createExpression();
		expression.setContent(content);
		Output connectorOutput = createOutput(content, String.class.getName());
	...
	public void shouldPerformBatchValidation_UseActorFilterDefStore() throws Exception {
		Connector connector = processFactory.createActorFilter();
		ConnectorConfiguration config = connectorConfigFactory.createConnectorConfiguration();
		config.setDefinitionId("myDef");
		config.setVersion("1.0.0");
		when(context.getTarget()).thenReturn(connector);
