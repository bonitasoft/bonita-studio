...
import org.bonitasoft.studio.application.handler.SwitchRepositoriesWorkspaceHandler;
...
public class CheckCurrentRepositoryState implements IPostStartupContribution {
    ...
    private void openMigrationRequiredDialog(AbstractRepository repository) {
        int buttonIndex = MessageDialog.open(MessageDialog.WARNING,
                ...
                SWT.NONE,
                Messages.migrate, org.bonitasoft.studio.application.i18n.Messages.switchRepository);
        if (buttonIndex == 0) {
            repository.runMigrationInDialog();
        } else {
            new CommandExecutor().executeCommand("org.bonitasoft.studio.repository.switch", Map.of());
        }
    }
    ...
    private void openSwitchProjectDialog(AbstractRepository repository) {
        MessageDialog.open(MessageDialog.ERROR,
                ...
                SWT.NONE,
                org.bonitasoft.studio.application.i18n.Messages.switchRepository);
        try {
