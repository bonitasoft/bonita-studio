...
public class RestApiZoomControl extends AbstractZoomControl {
	...
	public RestApiZoomControl(Composite parent, ZoomListener zoomListener, Dependency dep,
	...
	protected void createDetailsSection(Composite parent) {
		...
		composite.setLayout(GridLayoutFactory.fillDefaults().extendedMargins(10, 10, 10, 10).create());
		...
		iconLabel.setLayoutData(GridDataFactory.fillDefaults().align(SWT.END, SWT.FILL).span(1, 2)
				.hint(BonitaMarketplacePage.ICON_SIZE, BonitaMarketplacePage.ICON_SIZE).create());
		...
		if (restApiExtension.isPresent()) {
			try {
				var properties = bonitaDep.isProjectExtension() ? findPagePropertiesInProject() 
						: ExtensionPagePropertiesReader.getPageProperties(new File(restApiExtension.get().getArtifact().getFile())).orElseThrow();
	...
	private Properties findPagePropertiesInProject() {
		var bonitaProject = RepositoryManager.getInstance().getCurrentProject().orElseThrow();
		var extensionProject = bonitaProject.getExtensionsProjects().stream().filter(p -> {
			try {
				return Objects.equals(MavenProjectHelper.getMavenProject(p).getArtifactId(), dep.getArtifactId());
		...
		if (extensionProject != null) {
			var descriptor = extensionProject.getFile("page.properties");
			if (!descriptor.exists()) {
				descriptor = extensionProject.getFile("src/main/resources/page.properties");
			}
			if (descriptor != null) {
	...
	private void createRestAPIExtensionDetails(Composite parent, Properties properties, String extension) {
		String method = getProperty(properties, extension, PagePropertyConstants.METHOD)
				.orElse(Messages.httpMethodMissing);
		String path = getProperty(properties, extension, PagePropertyConstants.PATH_TEMAPLTE)
				.orElse(Messages.pathMissing);
		...
		createStyledText(composite, extension, method,
				JFaceResources.getFont(ProjectOverviewEditorPart.BOLD_0_FONT_ID));

		var detailsComposite = createComposite(composite, SWT.NONE);
		// num column is voluntary higher than the real num column (2), so we can have
		// alignement and we don't have a big white space between path and permissions
		...
		pathComposite.setLayout(GridLayoutFactory.fillDefaults().numColumns(3).create());
		...
		pathLabel.setFont(JFaceResources.getFont(ProjectOverviewEditorPart.BOLD_0_FONT_ID));
		...
		pathValueLabel.setFont(JFaceResources.getFont(ProjectOverviewEditorPart.ITALIC_0_FONT_ID));
		pathValueLabel.setData(BonitaThemeConstants.CSS_CLASS_PROPERTY_NAME, BonitaThemeConstants.CARD_BACKGROUND);

		new DynamicButtonWidget.Builder().withImage(Pics.getImage(PicsConstants.copyToClipboard))
				...
				.withCssclass(BonitaThemeConstants.CARD_BACKGROUND).createIn(pathComposite);
		...
		permissionsLabel.setFont(JFaceResources.getFont(ProjectOverviewEditorPart.BOLD_0_FONT_ID));
		...
		permissionListComposite.setLayoutData(GridDataFactory.fillDefaults().align(SWT.BEGINNING, SWT.FILL).create());
		...
		Arrays.asList(permissions.split(",")).forEach(permission -> {
			...
			permissionLabel.setFont(JFaceResources.getFont(ProjectOverviewEditorPart.ITALIC_0_FONT_ID));
	...
	private Optional<String> getProperty(Properties properties, String extension, String property) {
		return ExtensionPagePropertiesReader.getProperty(properties, String.format("%s.%s", extension, property));
	}
	...
	private void createStyledText(Composite parent, String extensionName, String extensionMethod, Font font) {
		...
		Display.getDefault().asyncExec(() -> {
			...
			nameStyle.font = font;
			StyleRange methodStyle = new StyleRange(extensionName.length() + 1, extensionMethod.length() + 2,
					styledText.getForeground(), styledText.getBackground());
			methodStyle.font = JFaceResources.getFont(ProjectOverviewEditorPart.ITALIC_0_FONT_ID);
