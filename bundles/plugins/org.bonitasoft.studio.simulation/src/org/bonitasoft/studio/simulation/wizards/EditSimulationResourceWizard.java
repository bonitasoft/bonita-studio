...
public class EditSimulationResourceWizard extends Wizard {
	 ...
	 * @param firstElement
	 */
	public EditSimulationResourceWizard(IRepositoryFileStore artifact) {
		this.artifact = artifact;
		this.resource = (Resource) artifact.getContent();
	...
	public boolean performFinish() {
		...
		String nameText = infoPage.getNameText();
		...
		Resource simulationResource = resource;
		...
		String timeUnit = infoPage.getTimeUnitText();
		...
		if(resource.getCalendar() == null){
			...
			resourceCalendar = (SimulationCalendar) resource.getCalendar() ;
		}
		...
		try {
			if(artifact != null) {
				...
				artifact = resourceStore.createRepositoryFileStore(simulationResource.getName()+"."+SimulationResourceRepositoryStore.SIMULATION_RESOURCE_EXT);
				artifact.save(simulationResource) ;
			}
		} catch (Exception e) {
