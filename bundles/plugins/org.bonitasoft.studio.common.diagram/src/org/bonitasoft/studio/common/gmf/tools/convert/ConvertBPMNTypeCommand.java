...
public class ConvertBPMNTypeCommand extends AbstractTransactionalCommand {
    ...
    public ConvertBPMNTypeCommand(final TransactionalEditingDomain domain, final EClass targetEClass,
            final GraphicalEditPart node, final ElementTypeResolver elementTypeResolver) {
     ...
     */
    @Override
    protected CommandResult doExecuteWithResult(final IProgressMonitor monitor, final IAdaptable adaptable)
        ...
        final CreateViewRequest activityRequest = CreateViewRequestFactory.getCreateShapeRequest(targetElementType,
        ...
        final GraphicalEditPart targetEditPart = (GraphicalEditPart) parentEditPart.findEditPart(parentEditPart,
                targetElement);

        if (ProcessPackage.eINSTANCE.getActivity().isSuperTypeOf(targetEClass)) {
            final ChangePropertyValueRequest fillReq = new ChangePropertyValueRequest("Fill Color",
                    Properties.ID_FILLCOLOR, ((FillStyle) node.getNotationView()
                            .getStyle(NotationPackage.eINSTANCE.getFillStyle())).getFillColor());
            targetEditPart.performRequest(fillReq);
            final ChangePropertyValueRequest lineReq = new ChangePropertyValueRequest("Line Color",
                    Properties.ID_LINECOLOR, ((LineStyle) node.getNotationView()
        ...
        final List<ICommand> commands = moveConnectionSourceAndTarget(node, editingDomain, parentEditPart,
                targetEditPart);
        ...
        try {
            final CopyEObjectFeaturesCommand operation = new CopyEObjectFeaturesCommand(editingDomain, sourceElement,
                    targetElement);
    ...
    protected void handleBoundaries(final IProgressMonitor monitor, final TransactionalEditingDomain editingDomain,
            final GraphicalEditPart parentEditPart,
        ...
        for (final BoundaryEvent boundaryEvent : ((Activity) targetElement).getBoundaryIntermediateEvents()) {
            final GraphicalEditPart boundaryEp = (GraphicalEditPart) targetEditPart.findEditPart(targetEditPart,
                    boundaryEvent);

            if (boundaryEp != null) {
                for (int i = 0; i < boundaryEvent.getOutgoing().size(); i++) {
                    final Connection outgoing = boundaryEvent.getOutgoing().get(i);
                    final GraphicalEditPart boundaryTargetEP = (GraphicalEditPart) parentEditPart
                            .findEditPart(parentEditPart.getRoot(), outgoing.getTarget());

                    if (boundaryTargetEP != null) {

                        final ConnectionEditPart ep = (ConnectionEditPart) GMFTools
                                .findEditPart((EditPart) parentEditPart.getRoot().getChildren().get(0), outgoing);
                        if (ep != null) {
                            try {
                        ...
                        final AbstractEMFOperation recreateExceptionFlowsOperation = new AbstractEMFOperation(
                                editingDomain, "Recreate Exception flow") {

                            @Override
                            protected IStatus doExecute(final IProgressMonitor monitor, final IAdaptable info)
                                    throws ExecutionException {
                                final IElementType elementType = ElementTypeRegistry.getInstance()
                                        .getType("org.bonitasoft.studio.diagram.SequenceFlow_4001");
                                final Edge edge = ViewService.getInstance().createEdge(elementType,
                                        GMFTools.getDiagramFor(ModelHelper.getMainProcess(targetElement),
                                                editingDomain),
                                        ((IHintedType) elementType).getSemanticHint(), -1, true,
                                ...
                                return new Status(IStatus.OK, "org.bonitasoft.studio.diagram.common",
                                        "Recreate Exception flow succeeded");
                            }
                        };
                        try {
    ...
    protected static void refreshBoundaryElements(final TransactionalEditingDomain editingDomain,
        ...
        for (final Pair<EObject, Point> pair : childPositions) {
            ...
            if (targetElement.eContents().contains(childToResetPosition)) {
                final GraphicalEditPart editPart = (GraphicalEditPart) targetEditPart.findEditPart(targetEditPart,
                ...
                if (editPart != null) {
                    ...
                    if (targetElement instanceof Activity) {
                         ...
                         */
                        final AbstractEMFOperation operation = new RemoveBoundaryWithItsFlows(editingDomain,
                                (BoundaryEvent) childToResetPosition,
    ...
    protected static List<ICommand> moveConnectionSourceAndTarget(final GraphicalEditPart node,
            final TransactionalEditingDomain editingDomain,
            final GraphicalEditPart parentEditPart, final GraphicalEditPart targetEditPart) {
        final List<ICommand> commands = new ArrayList<ICommand>();

        for (final org.eclipse.gmf.runtime.diagram.ui.editparts.ConnectionEditPart sourceConnection : (List<org.eclipse.gmf.runtime.diagram.ui.editparts.ConnectionEditPart>) node
            ...
            if (sourceConnection.resolveSemanticElement() instanceof MessageFlow) {
                if (!(targetEditPart.resolveSemanticElement() instanceof ThrowMessageEvent)) {
                    ...
                    editingDomain.getCommandStack().execute(
                            org.eclipse.emf.edit.command.DeleteCommand.create(editingDomain,
                                    sourceConnection.resolveSemanticElement()));
                    commands.add(new DeleteCommand(editingDomain, sourceConnection.getNotationView()));
                    continue;
                }
            }
            final SetConnectionEndsCommand setConnectionEndCommand = new SetConnectionEndsCommand(editingDomain,
                    Messages.setConnectionEndCommandLabel);
        ...
        for (final org.eclipse.gmf.runtime.diagram.ui.editparts.ConnectionEditPart targetConnection : (List<org.eclipse.gmf.runtime.diagram.ui.editparts.ConnectionEditPart>) node
            ...
            if (targetConnection.resolveSemanticElement() instanceof MessageFlow) {
                if (!(targetEditPart.resolveSemanticElement() instanceof AbstractCatchMessageEvent)) {
                    ...
                    editingDomain.getCommandStack().execute(
                            org.eclipse.emf.edit.command.DeleteCommand.create(editingDomain,
                                    targetConnection.resolveSemanticElement()));
        ...
        try {
            new CompositeCommand(Messages.setConnectionsCommandLabel, commands).execute(new NullProgressMonitor(),
                    parentEditPart);
