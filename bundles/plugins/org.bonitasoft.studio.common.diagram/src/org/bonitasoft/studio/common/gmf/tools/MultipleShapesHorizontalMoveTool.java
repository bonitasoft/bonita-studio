 ...
 * BonitasSoft S.A - Adaptation to BOS Model
 * Date Author Changes
 * Sep 11, 2007 Antoine Toulme Created
 * Dec 17, 2010 Romain Bioteau Modified for BOS Studio
 */
 ...
 */
public class MultipleShapesHorizontalMoveTool extends SimpleDragTracker {
    ...
    protected Command getCommand() {
        ...
        for (IGraphicalEditPart sp : _subProcesses) {
            ...
            SetBoundsCommand setBounds = new SetBoundsCommand(editingDomain, "MultipleShape Move", sp, spDim);
    ...
    protected int getCurrentPositionZoomed() {
        ...
        ((IGraphicalEditPart) ((IDiagramGraphicalViewer) getCurrentViewer()).getRootEditPart().getChildren().get(0))
    ...
    public static void setBoundsForOverlapComputation(IGraphicalEditPart ep, Rectangle result) {
        if (ep.resolveSemanticElement() instanceof FlowElement
                || ep.resolveSemanticElement() instanceof SubProcessEvent) {
    ...
    protected void updateSourceRequest() {
        ...
        ((ChangeBoundsRequest) getSourceRequest()).setMoveDelta(new Point(moved, 0));
        ((ChangeBoundsRequest) getSourceRequest()).setEditParts(_movingShapes);
    }

    @Override
    protected void executeCurrentCommand() {
        /*
         * Only one command must be executed :
         * use a GMF transactional command to wrap the original command
         * and the overlapping views adjustments.
         */
        Command originalCommand = getCurrentCommand();
        ICommandProxy overallCommand = new ICommandProxy(
                new AbstractTransactionalCommand(_container.getEditingDomain(), getCommandName(), null) {
                    ...
                    protected CommandResult doExecuteWithResult(IProgressMonitor arg0, IAdaptable arg1)
                        ...
                        // then, check overlap for all moving shapes
                        for (IGraphicalEditPart ep : _movingShapes) {
                            ...
                            Point newLoc = FiguresHelper.handleCompartmentMargin(ep, loc.getX(), loc.getY(),
                                    (ep.resolveSemanticElement() instanceof SubProcessEvent));
                            if ((newLoc.x != 0 && newLoc.y != 0) && !newLoc.equals(oldLoc)) {
                                // same behavior as SetBoundsCommand#doExecuteWithResult
                                EObjectAdapter adapter = new EObjectAdapter(ep.getNotationView());
                                View view = (View) adapter.getAdapter(View.class);
                                if (newLoc != null) {
                                    ViewUtil.setStructuralFeatureValue(view, NotationPackage.eINSTANCE.getLocation_X(),
                                            Integer.valueOf(newLoc.x));
                                    ViewUtil.setStructuralFeatureValue(view, NotationPackage.eINSTANCE.getLocation_Y(),
                                            Integer.valueOf(newLoc.y));
                                }
                            }
                        }
        ...
        setCurrentCommand(overallCommand);
