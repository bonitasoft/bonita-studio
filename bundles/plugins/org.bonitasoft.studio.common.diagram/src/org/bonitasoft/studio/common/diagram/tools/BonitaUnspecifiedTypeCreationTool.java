...
public class BonitaUnspecifiedTypeCreationTool extends UnspecifiedTypeCreationTool {
    ...
    public BonitaUnspecifiedTypeCreationTool(List<?> elementTypes) {
        super(elementTypes);
        if (!elementTypes.get(0)
                .equals(BonitaConnectionTypes.getElementType("org.bonitasoft.studio.diagram.Event_3024"))) { //$NON-NLS-1$
    ...
    protected boolean applies() {
        if (eClass != null) {
            return (ProcessPackage.Literals.FLOW_ELEMENT.isSuperTypeOf(eClass)
                    || ProcessPackage.Literals.BOUNDARY_EVENT.isSuperTypeOf(eClass)
            ...
            if (figure != null) {
                ...
                for (Object v : getCurrentViewer().getEditPartRegistry().keySet()) {
                    if (v instanceof Connector) {
                        ...
                        Rectangle bound = ((PolylineConnectionEx) ep.getFigure()).getSimpleBounds().getCopy()
                                .expand(new Insets(lineWidth, lineWidth, 0, 0));
    ...
    protected void executeCurrentCommand() {
        // chain the two commands to execute only once
        Command c = getCurrentCommand();
        var insert = new InsertElementOnSequenceFlowCommand(c, (IGraphicalEditPart) getTargetEditPart(),
                getCurrentViewer(), false);
        setCurrentCommand(c.chain(new ICommandProxy(insert)));
    ...
    public void setViewer(EditPartViewer viewer) {
        ...
        if (viewer != null && figure != null) {
            double zoom = ((DiagramRootEditPart) ((DiagramGraphicalViewer) getCurrentViewer()).getRootEditPart())
