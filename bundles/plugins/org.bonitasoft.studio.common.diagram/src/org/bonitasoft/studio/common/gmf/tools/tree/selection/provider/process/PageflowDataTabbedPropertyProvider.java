...
public class PageflowDataTabbedPropertyProvider extends ApplicationViewTabbedPropertySelectionProvider implements ITabbedSectionPropertyProvider {

    @Override
    public String tabId(final EObject element) {
        return isOverviewPageflowData(ModelHelper.getFirstContainerOfType(element, Data.class)) ? "tab.forms.overview" : "tab.forms.entry";
    }
    ...
    public boolean appliesTo(final EObject element, final IEditorReference activeEditor) {
        ...
        return data != null && isPageflowData(data);
    }

    private boolean isPageflowData(final Data data) {
        return isEntryPageflowData(data) || isOverviewPageflowData(data);
    }

    private boolean isEntryPageflowData(final Data element) {
        return element.eContainingFeature().equals(ProcessPackage.Literals.PAGE_FLOW__TRANSIENT_DATA);
    }

    private boolean isOverviewPageflowData(final EObject element) {
