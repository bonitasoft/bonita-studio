...
public class ValidatorsPropertySection extends AbstractBonitaDescriptionSection {
    ...
    private void createCreateValidatorButton(final Composite buttonsComposite) {
        ...
        importButton.addListener(SWT.Selection, new Listener() {
            ...
            public void handleEvent(final Event event) {
                ...
                final WizardDialog wd = new WizardDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), cvw);
        ...
        if(tableViewer != null && !tableViewer.getTable().isDisposed() && comboViewerForValidatorClass != null && !comboViewerForValidatorClass.getCombo().isDisposed()){
            ...
            if(lastEObject == null || !lastEObject.equals(getEObject())){
                ...
                if(lastEObject != null){
                    ...
                    final IObservableValue nameDetailValue = EMFObservables.observeDetailValue(Realm.getDefault(), treeViewerObservaleValue, FormPackage.Literals.VALIDATOR__NAME);
                    ...
                    final IObservableValue emfValidatorClassObservableDetailValue = EMFObservables.observeDetailValue(Realm.getDefault(), treeViewerObservaleValue, FormPackage.Literals.VALIDATOR__VALIDATOR_CLASS);

                    //Bind the Combo widget to the validator class detail
                    final IViewerObservableValue comboViewerObservable = ViewersObservables.observeSingleSelection(comboViewerForValidatorClass);
                    ...
                    modelStrategy.setConverter(new Converter(String.class,ValidatorDescriptor.class) {
                        ...
                        public Object convert(final Object from) {
                            if(from != null && from instanceof String){
                                final Collection<ValidatorDescriptor> input = (Collection<ValidatorDescriptor>) comboViewerForValidatorClass.getInput() ;
                    ...
                    final IObservableValue htmlDetailValue = EMFObservables.observeDetailValue(Realm.getDefault(), treeViewerObservaleValue, FormPackage.Literals.VALIDATOR__HTML_CLASS);
                    context.bindValue(SWTObservables.observeText(htmlClassField, SWT.Modify), htmlDetailValue);

                    final IObservableValue isBelowValue = EMFObservables.observeDetailValue(Realm.getDefault(), treeViewerObservaleValue, FormPackage.Literals.VALIDATOR__BELOW_FIELD);
        ...
        removeButton.addListener(SWT.Selection, new Listener() {
            ...
            public void handleEvent(final Event event) {
                ...
                if (tableViewer != null && ((IStructuredSelection) tableViewer.getSelection()).size() > 0) {
                    ...
                    final IUndoableOperation operation = new RemoveValidatorCommand(getEditingDomain(), getValidable(), selection, ValidatorsPropertySection.this);
                    try {
    ...
    private void createValidator() {
        final IUndoableOperation operation = new AddValidatorCommand(getEditingDomain(), getValidable(), generateValidatorDefaultName(), this);
        try {
            OperationHistoryFactory.getOperationHistory().execute(operation, new NullProgressMonitor(), null);
        } catch (final ExecutionException e) {
            BonitaStudioLog.error(e);
