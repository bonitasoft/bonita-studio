...
public class RemovedOutputChangeTest {
    ...
    public void should_not_be_a_breaking_change_if_output_is_not_referenced_in_connector_outputs() throws Exception {
        ...
        RemovedOutputChange change = new RemovedOutputChange(output);
    ...
    @Test
    public void should_remove_output_reference_from_connector_outputs_when_applying_change() throws Exception {
        // Given
        Output output = ConnectorDefinitionFactory.eINSTANCE.createOutput();
        output.setName("myOutput");
        RemovedOutputChange change = new RemovedOutputChange(output);
        ...
        // Then
        assertThat(connector.getOutputs().stream()
                .flatMap(op -> op.getRightOperand().getReferencedElements().stream())).isEmpty();
    }

    @Test
    public void should_remove_output_operation_from_connector_outputs_when_applying_change() throws Exception {
        // Given
        Output output = ConnectorDefinitionFactory.eINSTANCE.createOutput();
        output.setName("myOutput");
        RemovedOutputChange change = new RemovedOutputChange(output);
        Connector connector = aConnector()
                .havingOutput(anOperation()
                        .havingRightOperand(anExpression()
                                .withName("myOutput")
                                .withExpressionType(ExpressionConstants.CONNECTOR_OUTPUT_TYPE)
                                .havingReferencedElements(output)),
                        anOperation()
                                .havingRightOperand(anExpression()
                                        .withName("anotherOutput")
                                        .withExpressionType(ExpressionConstants.CONNECTOR_OUTPUT_TYPE)
                                        .havingReferencedElements(output)))
                .build();
        ...
        // Then
        assertThat(connector.getOutputs().stream().map(op -> op.getRightOperand())).extracting("name")
                .containsExactly("anotherOutput");
    }
