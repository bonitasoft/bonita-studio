 ...
 *
 */
public class RadioGroupItemEditingSupport extends EditingSupport {

    private final RadioGroup radioGroup;


    public RadioGroupItemEditingSupport(ColumnViewer viewer, RadioGroup radioGroup) {
        super(viewer);
    ...
    protected void setValue(Object element, Object value) {
        if(value != null){
            int i = radioGroup.getChoices().indexOf(element) ;
            radioGroup.getChoices().remove(element) ;
            if(i != -1){
                radioGroup.getChoices().add(i,value.toString()) ;
            }
            getViewer().refresh() ;
        }
    }
