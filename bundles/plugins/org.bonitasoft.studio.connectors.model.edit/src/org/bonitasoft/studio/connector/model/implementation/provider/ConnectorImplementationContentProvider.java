...
public class ConnectorImplementationContentProvider implements IStructuredContentProvider {
    ...
    public Object[] getElements(Object element) {
        final List<ConnectorImplementation> connectorImplList = ((IImplementationRepositoryStore) store).getImplementations() ;
        if(userImplementationOnly){
            List<ConnectorImplementation> toRemove = new ArrayList<ConnectorImplementation>() ;
        ...
        if(definitionId != null){
            final List<ConnectorImplementation> toRemove = new ArrayList<ConnectorImplementation>() ;
            for(ConnectorImplementation implementation : connectorImplList){
                if(implementation != null){
                    if(!implementation.getDefinitionId().equals(definitionId) || !implementation.getDefinitionVersion().equals(definitionVersion)){
                        toRemove.add(implementation) ;
                    }
                }
            }
