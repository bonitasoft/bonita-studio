 ...
 *
 */
public class CatchMessageEventCorrelationFilter implements IFilter {
	...
	public boolean select(Object toTest) {
		if (toTest instanceof IGraphicalEditPart) {
			...
			if(!(model instanceof StartMessageEvent)){
				return model instanceof AbstractCatchMessageEvent && !(model instanceof StartMessageEvent);
			}else{
				return ModelHelper.isInEvenementialSubProcessPool(model);
			}
