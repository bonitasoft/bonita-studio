...
public class CreateTargetEventDialog extends Dialog {
	...
	protected CreateTargetEventDialog(Shell parentShell,Element element) {
		super(parentShell);
	...
	protected Control createContents(Composite parent) {
		...
		comboProcess.setContentProvider(new ListContentProvider());
		...
		comboEvent.setContentProvider(new ListContentProvider());
	...
	private void refresh(){
		if(comboProcess.getCombo().getText() != null && comboProcess.getCombo().getText().length() >0){
			AbstractProcess proc = ModelHelper.findProcess(element,comboProcess.getCombo().getText());
