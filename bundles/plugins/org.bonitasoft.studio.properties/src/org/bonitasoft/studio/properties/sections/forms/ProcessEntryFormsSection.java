...
public class ProcessEntryFormsSection extends EntryFormsSection {
	...
	protected void refreshDataBinding() {
		...
		context.bindValue(SWTObservables.observeSelection(pageFlowRadio), EMFEditObservables.observeValue(getEditingDomain(), getPageFlow(), ProcessPackage.Literals.PAGE_FLOW__ENTRY_PAGE_FLOW_TYPE),
		...
		context.bindValue(SWTObservables.observeSelection(skipRadio), EMFEditObservables.observeValue(getEditingDomain(), getPageFlow(), ProcessPackage.Literals.PAGE_FLOW__ENTRY_PAGE_FLOW_TYPE),
				new UpdateValueStrategy() {
			@Override
			public Object convert(Object object) {
				return getPageFlowType() ;
