/**
 * noty - jQuery Notification Plugin v2.0.3 Contributors:
...
(function($) {
	...
	var NotyObject = {
	    ...
	    close : function() {
		    ...
		    if (!this.shown) { // If we are still waiting in the queue just
			    // delete from queue
		    ...
		    self.$bar.clearQueue().stop().animate(self.options.animation.close, self.options.animation.speed, self.options.animation.easing, function() {
			    ...
			    delete $.noty.store[self.options.id]; // deleting noty
			    // from store
