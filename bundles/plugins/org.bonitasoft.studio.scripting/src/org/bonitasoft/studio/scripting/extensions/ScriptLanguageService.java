...
public class ScriptLanguageService {
    ...
    public Set<IScriptLanguageProvider> getScriptLanguageProviders() {
        if (scriptProviders == null) {
            ...
            IConfigurationElement[] elements = BonitaStudioExtensionRegistryManager.getInstance()
                    .getConfigurationElements(SCRIPT_LANGUAGE_EXTENSION_ID);
            for (IConfigurationElement element : elements) {
                try {
                    IScriptLanguageProvider provider = (IScriptLanguageProvider) element
                            .createExecutableExtension(PROVIDER_CLASS_ATTRIBUTE);
    ...
    public String getDefaultLanguage() {
        return getScriptLanguageProviders().isEmpty() ? null
                : getScriptLanguageProviders().iterator().next().getLanguageId();
    }
