...
public class BusinessObjectBOSArchiveProviderTest {
    ...
    public void shouldGetFileStoreForConfiguration_ReturnBusinessObjectFileStore() throws Exception {
        when(store.getChildByQualifiedName("org.bonita.Car")).thenReturn(fileStore);
        assertThat(providerUnderTest.getFileStoreForConfiguration(processWithABusinessObjectData, null)).isNotEmpty().hasSize(1).containsOnly(fileStore);
    }
