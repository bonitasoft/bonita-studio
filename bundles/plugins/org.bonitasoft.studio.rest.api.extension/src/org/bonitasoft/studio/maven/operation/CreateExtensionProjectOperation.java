...
/**
 * Operation to create an extension project.
 * The new project must be created from a maven archetype.
 */
public abstract class CreateExtensionProjectOperation extends AbstractMavenProjectUpdateOperation {

    private static final String UTF_8 = "UTF-8";
    private final IStatus status = Status.OK_STATUS;
    private List<IProject> projects;
    private final ExtensionProjectArchetypeConfiguration archetypeConfiguration;
    ...
    protected IProject doRun(final IProgressMonitor monitor) throws CoreException {
        ...
        projects = MavenPlugin.getProjectConfigurationManager()
                .importProjects(mavenProjects, projectImportConfiguration, project -> {
                    try {
                        // configure project before adding to app, so it can build correctly for triggered analysis
    ...
    protected void addDependencyToAppProject() throws CoreException {
        ...
        var existingDependency = appModel.getDependencies()
                ...
                .filter(dep -> Objects.equals(dep.getArtifactId(), archetypeConfiguration.getProjectName()))
                .filter(dep -> Objects.equals(dep.getType(), "jar") || dep.getType() == null)
                .findFirst();
        if (existingDependency.isEmpty()) {
            extensionDependency.setGroupId("${project.groupId}");
            extensionDependency.setArtifactId(archetypeConfiguration.getProjectName());
            extensionDependency.setVersion("${project.version}");
            extensionDependency.setType(null);
    ...
    private void refreshProjectExplorerView() {
        ...
        if (viewPart != null) {
            ...
            var store = RepositoryManager.getInstance().getCurrentRepository().orElseThrow()
