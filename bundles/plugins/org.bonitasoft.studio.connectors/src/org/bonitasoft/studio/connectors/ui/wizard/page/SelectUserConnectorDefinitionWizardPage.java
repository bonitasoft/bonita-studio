...
public class SelectUserConnectorDefinitionWizardPage extends SelectConnectorDefinitionWizardPage {
    ...
    private void deleteConnectorRessources() {
        if (selection != null) {
            ...
            if (FileActionDialog.confirmDeletionQuestionWithCustomMessage(
                    Messages.bind(Messages.deleteConnectorDefinition, fileName))) {
    ...
    private void deleteConnectorConfigurations() {
        if (selection != null) {
            final List<ConnectorConfiguration> connectorConfs = connectorConfStore
                    .getConnectorConfigurationsFor(selection.getId(), selection.getVersion());
            for (final ConnectorConfiguration conf : connectorConfs) {
                final DefinitionConfigurationFileStore file = connectorConfStore
                        .getChild(conf.getName() + "." + ConnectorConfRepositoryStore.CONF_EXT, true);
                if (file != null) {
                    file.delete();
                }
            }
    ...
    private void deleteConnectorImplementations() {
        if (selection != null) {
            final List<ConnectorImplementation> connectorImpls = connectorImplStore
                    .getImplementations(selection.getId(), selection.getVersion());
            for (final ConnectorImplementation connectorImpl : connectorImpls) {
                connectorImplStore.getImplementationFileStore(connectorImpl.getImplementationId(),
                        connectorImpl.getImplementationVersion()).delete();
            }
        }
    }
