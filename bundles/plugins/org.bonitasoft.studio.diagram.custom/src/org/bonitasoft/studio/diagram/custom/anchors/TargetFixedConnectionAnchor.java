...
public class TargetFixedConnectionAnchor extends AbstractConnectionAnchor {
	...
	public TargetFixedConnectionAnchor(
			GraphicalEditPart ep,
			ConnectionEditPart connEditPart) {
		this(ep,((IGraphicalEditPart)connEditPart.getSource()).getFigure());
	...
	public Point getLocation(Point reference) {
		...
		FiguresHelper.translateToAbsolute(sourceFig, source);
		FiguresHelper.translateToAbsolute(owner, target);

		if(sourceFig.equals(targetFig)){
			return AnchorUtil.getSouthLocation(getOwner());
		}
		...
		if(ep.resolveSemanticElement() instanceof MessageEvent && connEditPart != null && connEditPart instanceof MessageFlowEditPart){
			int position = AnchorUtil.getMessageEventAnchorPosition(target,source);
			switch (position) {
			...
			int position = AnchorUtil.getEventAnchorPosition(target,source);
			switch (position) {
			...
			int position = AnchorUtil.getGatewayAnchorPosition(target,source);
			switch (position) {
			case AnchorPosition.NORTH:return computeGateAvailableAnchor(AnchorPosition.NORTH,freeAnchors);
			case AnchorPosition.SOUTH:return computeGateAvailableAnchor(AnchorPosition.SOUTH,freeAnchors);
			...
			int position = AnchorUtil.getAnchorPosition(target,source);
