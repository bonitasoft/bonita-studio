...
public class ExtractSubprocess extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		List<IGraphicalEditPart> parts = GMFTools.addMissingConnectionsAndBoundaries(editor.getDiagramGraphicalViewer().getSelectedEditParts());
		List<String> errors = evaluateErrors(parts);
		if (!errors.isEmpty()) {
			MessageDialog.openError(editor.getSite().getShell(), Messages.extractSubprocessError, createErrorMessage(errors));
		} else {
			createSubprocess(parts);
		}
		return null;
	}
	 ...
	 * @param parts
	 */
	private void createSubprocess(List<IGraphicalEditPart> parts) {
		...
		int i  = NamingUtils.getMaxElements(processSemantic,EXTRACTED_SUBPROCESS);
		i++;
		...
		cc.add(createSetCommand(createSubprocessNodeReq, ProcessPackage.Literals.CALL_ACTIVITY__CALLED_ACTIVITY_NAME, createStringExpression(extractedSubprocessName), processSemantic));
		...
		for (IGraphicalEditPart part :GMFTools.addMissingConnectionsAndBoundaries(parts)) {
			if (! (part instanceof ConnectionEditPart) ||
					(part instanceof ConnectionEditPart && ((ConnectionEditPart)part).getSource() != newSubprocessPart && ((ConnectionEditPart)part).getTarget() != newSubProcess)) {
	...
	private Expression createStringExpression(String value) {
