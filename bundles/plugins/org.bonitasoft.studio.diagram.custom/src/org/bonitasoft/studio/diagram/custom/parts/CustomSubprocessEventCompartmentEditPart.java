...
public class CustomSubprocessEventCompartmentEditPart extends SubProcessEventSubProcessCompartment2EditPart {
    ...
    protected void updateBackgroundColor(boolean collapsed) {
        CollapsableEventSubprocessFigure figure = (CollapsableEventSubprocessFigure) ((IGraphicalEditPart)getParent()).getContentPane() ;
        if(!collapsed){
            beforeExpandColor = (Integer) ((IGraphicalEditPart) getParent()).getStructuralFeatureValue(NotationPackage.eINSTANCE.getFillStyle_FillColor()) ;
            ChangePropertyValueRequest colorReq = new ChangePropertyValueRequest("Fill Color", Properties.ID_FILLCOLOR, FigureUtilities.colorToInteger(ColorConstants.white)) ;
            ...
            if(beforeExpandColor == null){
                colorReq = new ChangePropertyValueRequest("Fill Color", Properties.ID_FILLCOLOR, ((IGraphicalEditPart) getParent()).getPreferredValue(NotationPackage.eINSTANCE.getFillStyle_FillColor())) ;
            }else{
    ...
    protected void updateCollapse(boolean collapsed) {
        ...
        if(collapsed){
            if(((IGraphicalEditPart)getParent()).getFigure().getBounds().width == beforeExpandWidth && beforeExpandWidth!= 0){
        ...
        if(!collapsed){
            ...
            beforeCollapseWidth = Math.round((float)(((IGraphicalEditPart)getParent()).getFigure().getBounds().width * zm.getZoom()));
            beforeCollapseHeight = Math.round((float)(((IGraphicalEditPart)getParent()).getFigure().getBounds().height * zm.getZoom()));
            ...
            List<IGraphicalEditPart> rightEps =  getChildrenToMoveHorizontaly( bounds.x + bounds.width, bounds.y + bounds.height,zm.getZoom()) ;
            List<IGraphicalEditPart> bottomEps =  getChildrenToMoveVerticaly(bounds.x , bounds.y+bounds.height ,zm.getZoom()) ;
    ...
    private int getMaxHeight() {
        ...
        for(Object child : getChildren()){
            if(child instanceof IGraphicalEditPart){
                int childHeight =  ((IGraphicalEditPart) child).getFigure().getBounds().y +((IGraphicalEditPart) child).getFigure().getBounds().height ;
                if(childHeight > max){
                    max = childHeight ;
                }
            }
    ...
    private int getMaxWidth(){
        ...
        for(Object child : getChildren()){
            if(child instanceof IGraphicalEditPart){
                int childWidth = ((IGraphicalEditPart) child).getFigure().getBounds().x + ((IGraphicalEditPart) child).getFigure().getBounds().width ;
                if(childWidth > max){
                    max = childWidth ;
                }
            }
