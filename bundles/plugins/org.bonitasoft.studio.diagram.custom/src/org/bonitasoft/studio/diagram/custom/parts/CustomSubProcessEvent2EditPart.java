		...
		addEditPartListener(new EditPartListener(){
			...
			public void childAdded(EditPart child, int index) {
				if(child instanceof CustomSubprocessEventCompartmentEditPart){
					if(!((BasicCompartment)((CustomSubprocessEventCompartmentEditPart)child).getNotationView()).isCollapsed()){
						((CollapsableEventSubprocessFigure)getContentPane()).setUseGradient(false) ;
						((CustomSubprocessEventCompartmentEditPart)child).setBeforeExpandColor((Integer) getPreferredValue(NotationPackage.eINSTANCE.getFillStyle_FillColor())) ;
