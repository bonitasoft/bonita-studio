 ...
 *
 */
public class LoopSubprocessTaskDecorator extends AbstractBonitaDecorator {
	
	
	public LoopSubprocessTaskDecorator(IDecoratorTarget decoratorTarget, ActivityDecoratorProvider activityDecoratorProvider) {
	...
	protected DecoratorSVGFigure getImageDecorator() {
		DecoratorSVGFigure figure = FiguresHelper.getDecoratorFigure(FiguresHelper.LOOP_SUB_DECORATOR) ;
	...
	protected boolean isAppearing(EObject activity) {
		return activity instanceof CallActivity && ((CallActivity)activity).getIsLoop() ;
	}
