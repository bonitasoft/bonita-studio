...
public class HtmlTemplateGenerator {
     ...
     */
    public List<String> createXhtmlTemplate(List<EObject> toGenerate, boolean isPreview,
            ApplicationLookNFeelFileStore artifact, boolean instantiationForm) throws CoreException {
            ...
            try {
                ...
                try (FileInputStream fis = new FileInputStream(templateFile);) {
                    ...
                    if (instantiationForm || form.eContainer() instanceof AbstractProcess) {
                        ...
                        HashSet<net.htmlparser.jericho.Element> hashSet = new HashSet<net.htmlparser.jericho.Element>(
                                headers);
                        hashSet.add(element);
    ...
    private List<String> generateFormContainerDivs(List<EObject> toGenerate) {
        URI transformationURI = URI.createPlatformPluginURI("/org.bonitasoft.studio.exporter/transforms/Form2Xhtml.qvto", //$NON-NLS-1$
                false);
        ...
        try {
            ...
            for (EObject eObject : toGenerate) {
                ...
                File htmlFile = new File(
                        TMP_DIR + File.separatorChar + ExporterTools.getPageTemplateWarFileName((Form) eObject));
                htmlFile.delete();
                ...
                try (FileOutputStream os = new FileOutputStream(htmlFile);) {
        ...
        for (EObject eObject : toGenerate) {
            ...
            if (eObject.eContainer() != null) {
                ...
                if (eObject.eContainmentFeature().equals(ProcessPackage.Literals.VIEW_PAGE_FLOW__VIEW_FORM)) {
                    newForm.setDocumentation(ModelHelper.getEObjectID(eObject.eContainer()) + suffix + "_"
                            + ExporterTools.CONSULTATION + ModelHelper.getEObjectID(eObject));
                } else if (eObject.eContainmentFeature().equals(ProcessPackage.Literals.RECAP_FLOW__RECAP_FORMS)) {
                    newForm.setDocumentation(ModelHelper.getEObjectID(eObject.eContainer()) + suffix + "_"
                            + ExporterTools.RECAP + ModelHelper.getEObjectID(eObject));
                } else {
                    newForm.setDocumentation(ModelHelper.getEObjectID(eObject.eContainer()) + suffix + "_"
     ...
     * @return
     */
    public String createXhtmlTemplate(Form form, boolean isPreview, ApplicationLookNFeelFileStore artifact,
            boolean instantiationForm) {
     ...
     * @throws IOException
     */
    public void changeDivId(String oldId, String newId, FileInputStream pageTemplate, FileWriter outputFileWriter)
            throws IOException {
        Source pageTemplateSource = new Source(pageTemplate);
        OutputDocument resultingTemplate = new OutputDocument(pageTemplateSource);
