 ...
 * @generated
 */
public class SubProcessEventItemSemanticEditPolicy extends ProcessBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public SubProcessEventItemSemanticEditPolicy() {
		super(ProcessElementTypes.SubProcessEvent_2031);
	}
				...
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					...
					case SendTask2EditPart.VISUAL_ID:
						...
						for (Iterator<?> it = cnode.getSourceEdges().iterator(); it.hasNext();) {
							...
							if (ProcessVisualIDRegistry.getVisualID(outgoingLink) == MessageFlowEditPart.VISUAL_ID) {
								DestroyElementRequest r = new DestroyElementRequest(outgoingLink.getElement(), false);
								cmd.add(new DestroyElementCommand(r));
								cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
								continue;
					...
					case TextAnnotation2EditPart.VISUAL_ID:
						...
						for (Iterator<?> it = cnode.getSourceEdges().iterator(); it.hasNext();) {
							Edge outgoingLink = (Edge) it.next();
							if (ProcessVisualIDRegistry
									.getVisualID(outgoingLink) == TextAnnotationAttachmentEditPart.VISUAL_ID) {
								DestroyElementRequest r = new DestroyElementRequest(outgoingLink.getElement(), false);
								cmd.add(new DestroyElementCommand(r));
								cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
								continue;
