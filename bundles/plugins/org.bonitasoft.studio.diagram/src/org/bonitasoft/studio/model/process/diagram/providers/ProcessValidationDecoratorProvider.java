...
public class ProcessValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	* @generated
	*/
	private static final String MARKER_TYPE = ProcessDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$
		...
		public void refresh() {
			...
			if (editPart instanceof org.eclipse.gef.GraphicalEditPart) {
				if (view instanceof Edge) {
					...
					if (view.getElement() instanceof Container) {
						setDecoration(getDecoratorTarget().addShapeDecoration(getImage(severity),
								IDecoratorTarget.Direction.NORTH_WEST, margin, true));
					} else {
						setDecoration(getDecoratorTarget().addShapeDecoration(getImage(severity),
								IDecoratorTarget.Direction.SOUTH_EAST, -3, true));
					}
				}
