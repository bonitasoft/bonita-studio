...
import org.bonitasoft.studio.model.process.diagram.edit.commands.BoundaryMessageEvent2CreateCommand;
import org.bonitasoft.studio.model.process.diagram.edit.commands.BoundarySignalEvent2CreateCommand;
import org.bonitasoft.studio.model.process.diagram.edit.commands.BoundaryTimerEvent2CreateCommand;
import org.bonitasoft.studio.model.process.diagram.edit.commands.IntermediateErrorCatchEventCreateCommand;
import org.bonitasoft.studio.model.process.diagram.edit.commands.NonInterruptingBoundaryTimerEvent2CreateCommand;
 ...
 * @generated
 */
public class CallActivity2ItemSemanticEditPolicy extends ProcessBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public CallActivity2ItemSemanticEditPolicy() {
		super(ProcessElementTypes.CallActivity_3063);
	}
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (ProcessVisualIDRegistry.getVisualID(incomingLink) == SequenceFlowEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (ProcessVisualIDRegistry.getVisualID(outgoingLink) == SequenceFlowEditPart.VISUAL_ID) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			case IntermediateErrorCatchEventEditPart.VISUAL_ID:
				for (Iterator<?> it = node.getTargetEdges().iterator(); it.hasNext();) {
					Edge incomingLink = (Edge) it.next();
					if (ProcessVisualIDRegistry
							.getVisualID(incomingLink) == TextAnnotationAttachmentEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(incomingLink.getElement(), false);
						cmd.add(new DestroyElementCommand(r));
						cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
						continue;
			...
			case BoundaryMessageEvent2EditPart.VISUAL_ID:
				for (Iterator<?> it = node.getTargetEdges().iterator(); it.hasNext();) {
					Edge incomingLink = (Edge) it.next();
					if (ProcessVisualIDRegistry.getVisualID(incomingLink) == MessageFlowEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(incomingLink.getElement(), false);
						cmd.add(new DestroyElementCommand(r));
						cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
						continue;
