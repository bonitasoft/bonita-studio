...
public class ProcBuilder implements IProcBuilder {
	 ...
	 */
	public void addSequenceFlow(String name, final String sourceId, final String targetId, final boolean isDefault,final Point sourceAnchor, final Point targetAnchor, final PointList bendpoints) throws ProcBuilderException {
	...
	public void addSequenceFlowCondition(final String content, final String interpreter, final String expressionType){
		Expression condition = createExpression(content, Boolean.class.getName(), interpreter, expressionType);
	...
	private Element createShape(String id, EObject container, Point location, Dimension size, IElementType type, boolean isCollapsed) throws ProcBuilderException {
		...
		if(compartment instanceof LaneLaneCompartmentEditPart){
			...
			PoolPoolCompartmentEditPart poolCompartment = (PoolPoolCompartmentEditPart) compartment.getParent().getParent();
			int currentLaneIndex = ((Container)currentContainer.eContainer()).getElements().indexOf(currentContainer);
			if(currentLaneIndex > 0 ){
				for(Object object : poolCompartment.getChildren()){
					if(object instanceof LaneEditPart){
						EObject semanticElement = ((LaneEditPart) object).resolveSemanticElement();
						int laneIndex = ((Container)currentContainer.eContainer()).getElements().indexOf(semanticElement);
