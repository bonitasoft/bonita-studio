...
public class TestLookAndFeel extends TestCase {
	...
	public void testDownloadDefaultTemplate() {
		...
		tester.downloadDefaultTemplate(t, ResourcesPlugin.getWorkspace()
				.getRoot().getLocation().toFile().getAbsolutePath()
				+ File.separatorChar + fileName);
		final File createdFile = new File(ResourcesPlugin.getWorkspace()
				.getRoot().getLocation().toFile().getAbsolutePath()
				+ File.separatorChar + fileName);
		assertNotNull("File " + fileName
				+ " is asked to be downloaded but is not", createdFile);
		...
		try {
			...
			fail("File " + fileName
					+ " were not present in the destination folder");
		}
		createdFile.delete();
	}
}
