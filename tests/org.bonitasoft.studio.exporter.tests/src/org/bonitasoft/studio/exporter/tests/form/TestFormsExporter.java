...
public class TestFormsExporter extends TestCase {
    ...
    public void testFormsXmlActions() throws Exception {
        ...
        Object result = xpathEvaluator.evaluate("//pages/page[@id='Step1']/actions/action/variable", document,
                XPathConstants.STRING);
        assertEquals("plop", result);
        result = xpathEvaluator.evaluate("//pages/page[@id='Step1']/actions/action/expression/name", document,
                XPathConstants.STRING);
        assertEquals("field_plop", result);
        result = xpathEvaluator.evaluate("//pages/page[@id='Step1']/actions/action/expression/expression-content", document,
                XPathConstants.STRING);
        assertEquals("field_plop", result);
        result = xpathEvaluator.evaluate("//pages/page[@id='Step1']/actions/action/expression/expression-type", document,
                XPathConstants.STRING);
        assertEquals(ExpressionConstants.FORM_FIELD_TYPE, result);
        result = xpathEvaluator.evaluate("//pages/page[@id='Step1']/actions/action/expression/return-type", document,
                XPathConstants.STRING);
    ...
    public void testFormsXmlVariableBound() throws Exception {
        ...
        Object result = xpathEvaluator.evaluate("//pages/page[@id='Step1']/actions/action[variable-type='"
                + ExpressionConstants.LEFT_OPERAND_DOCUMENT + "']/variable", document, XPathConstants.STRING);
    ...
    public void testSkipForms() throws Exception {
        ...
        Object result = xpathEvaluator.evaluate("//forms/form[@id='testSkipForms--1.0$entry']", document,
                XPathConstants.NODE);
        Node pageflow = (Node) result;
        assertNotNull(pageflow);
        assertTrue("PageFlow for testSkipForms--1.0$entry has " + pageflow.getChildNodes().getLength()
                + "children but shouldn't", pageflow.getChildNodes().getLength() == 5);//1 for permission + 1 for fomr-type + 3 for th return to line
        ...
        result = xpathEvaluator.evaluate("//forms/form[@id='testSkipForms--1.0--Step1$entry']", document,
                XPathConstants.NODE);
