...
public class  BonitaTitleBarPresentation extends StackPresentation{
	...
	protected static final int TOOLBAR_WIDTH = 85;
	 ...
	 * @param showToolbar 
	 */
	public BonitaTitleBarPresentation(Composite parent, 
			IStackPresentationSite stackSite, boolean showTb) {
		super(stackSite);
		this.showToolbar = showTb ;
				...
				if(current != null) {
					...
					if (textWidth > 0) {
						...
						if(!showToolbar){
							...
							gc.drawText(text,TOOLBAR_WIDTH, textY, SWT.DRAW_TRANSPARENT | SWT.DRAW_MNEMONIC);					
						}
					}				
				}

				//contentArea.setBackgroundImage(Pics.getImage(PresentationImages.COOLBAR_BACKGROUND)); //$NON-NLS-1$
			}
		});
			...
			public void widgetSelected(SelectionEvent e) {
				if (current != null) {
					if(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getPages().length>0){
						if(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getPages()[0].getEditorReferences().length > 0){
	...
	private void layout() {
		...
		if(showToolbar){
			if (current != null) {
				Rectangle clientRectangle = presentationControl.getBounds();
				...
				if(current.getToolBar() != null){
					current.getToolBar().setBounds(0,2, TOOLBAR_WIDTH, titleArea.getBounds().height-2) ;
					current.getToolBar().setBackgroundImage(Pics.getImage(PresentationImages.COOLBAR_BACKGROUND)) ;
				}
			}
		}else{
			if (current != null) {
