...
public class ContentAssistText extends Composite implements SWTBotConstants {
	...
	private Path getBorderPath(Rectangle widgetBounds, Display display) {
		...
		path.addRectangle(-1,-1,widgetBounds.width,2);//Top border
		path.addRectangle(0, widgetBounds.height, widgetBounds.width,2);//Bottom border
		path.addRectangle(widgetBounds.width, 0, 2,widgetBounds.height);//Right border
		return path;
	}
