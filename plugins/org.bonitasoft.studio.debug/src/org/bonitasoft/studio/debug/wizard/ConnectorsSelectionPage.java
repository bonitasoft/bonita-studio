...
public class ConnectorsSelectionPage extends WizardPage {
    ...
    protected void createTreeViewer(Composite mainComposite) {
        ...
        //        ModelHelper.findAllConnectors(mainProcess, input) ;
        //        final List<CallActivity> callActivities = ModelHelper.getAllItemsOfType(mainProcess, ProcessPackage.Literals.CALL_ACTIVITY);
        ...
        //                        ModelHelper.findAllConnectors(proc, input) ;
        //                    }
        //                }else{
    ...
    private void retrieveAllConnectors(MainProcess diagram, List<EObject> result) {
        ...
        final List<CallActivity> callActivities = ModelHelper.getAllItemsOfType(diagram, ProcessPackage.Literals.CALL_ACTIVITY);
        final DiagramRepositoryStore store = (DiagramRepositoryStore) RepositoryManager.getInstance().getRepositoryStore(DiagramRepositoryStore.class);
        final List<AbstractProcess> allProcesses = store.getAllProcesses();
        for(CallActivity activity : callActivities){
            final String procName = activity.getCalledActivityName().getContent();
            final String procVersion = activity.getCalledActivityVersion().getContent();
            for(AbstractProcess proc : allProcesses){
                if(!ModelHelper.getMainProcess(proc).equals(diagram)){
