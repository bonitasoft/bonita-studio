...
public class ApplicationResourcesProvider implements BARResourcesProvider {
    ...
    public List<BarResource> addResourcesForConfiguration(BusinessArchiveBuilder builder,AbstractProcess process,Configuration configuration) {
        List<BarResource> res = new ArrayList<BarResource>();
        try {
            ...
            Display.getDefault().syncExec(new Runnable() {
                @Override
                public void run() {
                    String message = Messages.exportFailedMessage ;
                    if(e.getMessage() != null && !e.getMessage().isEmpty()){
                        message = e.getMessage() ;
                    }
                    new BonitaErrorDialog(Display.getDefault().getActiveShell(), Messages.exportFailedTitle, message, e).open() ;
    ...
    protected void addApplicationResources(List<BarResource> res, AbstractProcess process) throws Exception {
        ...
        if (resourceFile.exists()) {
            ...
            PlatformUtil.delete(resourceFile, new NullProgressMonitor());
        } else {
