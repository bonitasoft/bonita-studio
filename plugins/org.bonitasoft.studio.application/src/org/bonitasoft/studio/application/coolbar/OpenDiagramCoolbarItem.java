...
public class OpenDiagramCoolbarItem implements IBonitaContributionItem {
	...
	private Command getCommand() {
		...
		Command cmd = service.getCommand("org.bonitasoft.studio.diagram.command.openDiagram") ;
		return cmd;
	}
	...
	public void fill(ToolBar toolbar, int index, int iconSize) {
		...
		if(iconSize < 0 ){
			...
			item.setImage(Pics.getImage("/editIcons/open.png")) ;
		}else{
			smallImage = new Image(Display.getDefault(),Pics.getImage("/editIcons/open.png").getImageData().scaledTo(iconSize, iconSize));
