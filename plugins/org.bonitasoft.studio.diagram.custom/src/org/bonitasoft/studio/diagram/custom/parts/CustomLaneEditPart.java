...
public class CustomLaneEditPart extends LaneEditPart {
	...
	private void logWrongParent() {
		...
		String currentLaneName = resolvedSemanticElement instanceof Element ? ((Element) resolvedSemanticElement).getName() : resolvedSemanticElement.toString();
		EObject parentResolvedSemanticElement = ((GraphicalEditPart) getParent()).resolveSemanticElement();
		String containerName = parentResolvedSemanticElement instanceof Element ? ((Element) parentResolvedSemanticElement).getName() : parentResolvedSemanticElement.toString();
		BonitaStudioLog.warning("Warning: the lane"+currentLaneName+" is contained in another thing than a PoolCompartment. It is contained in: "+containerName, Activator.PLUGIN_ID);
	}
	...
	private int computeLaneHeight(List<CustomLaneEditPart> poolLanes) {
		...
		if(poolLanes.size() ==  0 && ((Integer) getStructuralFeatureValue(NotationPackage.eINSTANCE.getSize_Height())).intValue() == -1){
			...
			if(height == 0){
				height = ((Integer) ((CustomPoolEditPart)getParent().getParent()).getStructuralFeatureValue(NotationPackage.eINSTANCE.getSize_Height())).intValue();
			}
